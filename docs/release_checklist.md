# MitoOverview v0.3.1 Release Checklist

This checklist governs the GitHub-primary v0.3.1 software release. The patch
changes release/report tooling only and reuses the scientific protocol defined in
[`clean_room_validation_protocol_v0.3.0.md`](clean_room_validation_protocol_v0.3.0.md).
The `v0.3.0` tag remains immutable at
`b116430e037f4ff2f9cf6f6f3ba66150cce1303f`; its scientific inputs,
selection seed, thresholds, schemas, oracles, and normalized results are frozen.
...
- [ ] `pyproject.toml`, `mito_overview.__version__`, `CITATION.cff`, README,
  and CHANGELOG agree on version `0.3.1`.
...
- [ ] Only after the external digest gate passes, `verify_bundle.sh` passes in
  ...
  `mito-overview-v0.3.1-validation.zip`. This verifier proves internal
...
- [ ] Annotated tag `v0.3.1` peels to `FINAL_SHA` and is never rewritten.
- [ ] `scripts/run_fresh_public_tag_validation_v0.3.1.sh` clones the public
...
- [ ] The draft GitHub release targets the existing `v0.3.1` tag.
...
- [ ] `scripts/build_release_validation_report_v0.3.1.py` emits
...
- [ ] Every DOCX page is rendered to `page-<N>.png` and inspected. The PDF and
  ...
  `scripts/finalize_release_validation_report_v0.3.1.py` with an explicit PASS
...
- [ ] `scripts/assemble_release_assets_v0.3.1.py` creates the exact eleven-file
  ...
  report-asset manifest in `mito-overview-v0.3.1-verification.json`.
...
publication is corrected forward as `v0.3.2`, never by moving `v0.3.1`.
