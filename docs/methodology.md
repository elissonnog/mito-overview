...
Candidate-evaluable positions additionally require a canonical `A/C/G/T` reference base. A reference with no canonical mitochondrial positions yields `not_evaluable/no_canonical_reference_positions`; noncanonical positions are counted explicitly and cannot contribute an apparent whole-mitochondrion zero-candidate result. Configuration and observation-policy thresholds must be finite and within their documented domains before pileup begins.
...
Circularity edge-depth QC applies the same input-quality invariant to `mito_depth_per_base.tsv`: the coordinate inventory must be exactly the integers `1..MT_LENGTH`, each once, and all depths and calculated regional means must be finite and nonnegative. A present but malformed profile is retained as evidence but yields `not_evaluable/incomplete_depth_profile` and `NA` edge/interior means; positions are never coerced by truncating fractional coordinates. Candidate-edge fractions consume the complete candidate-observation contract described below, including unique in-range coordinates. A missing candidate file or a valid complete-schema file with zero rows yields a metric-level `NA/not_evaluable`; any existing file, including a zero-row file, is validated before empty-result handling, so a zero-byte or partial-header table stops the step rather than supporting an interpreted fraction. Primary-read edge metrics require an exact binary primary indicator, finite integer start/end coordinates in `1..MT_LENGTH`, ordered start/end pairs for the combined soft-clip metric, and finite soft-clip fractions in `[0,1]`. Invalid optional read evidence yields metric-level `NA/not_evaluable` with an explicit reason code; it does not overwrite the independent status of a valid depth profile.

## Reference and NUMT interpretation gates
Automatic whole-genome scope requires exact and concordant GRCh37, GRCh38, GRCm38, or GRCm39 chromosome-length profiles in both the FASTA index and alignment header, including the assembly-specific mitochondrial length and no extra contigs. Reduced, scaled, hybrid, augmented, discordant, or modified profiles resolve conservatively and cannot enable categorical NUMT interpretation; mt-only references resolve to `mt_only`. CRAM preflight compares mitochondrial sequence-dictionary MD5 metadata with the supplied reference sequence even when the CRAM contains no mitochondrial records. Categorical NUMT-warning output is additionally withheld if required read-stat columns, valid primary-alignment indicators, at least one primary alignment, or the near-complete aligned-reference QC metric are unavailable. MAPQ must be a finite integer in `[0,254]`; `255` is the SAM unavailable value and is not treated as high-quality evidence. Query length must be a positive integer, soft-clipped bases must be a nonnegative integer no greater than query length, `aligned_reference_bases + softclip_bases` must not exceed query length, and the reported soft-clip fraction must agree with `softclip_bases/query_length`. Mitochondrial aligned-reference and soft-clip fractions must be finite in `[0,1]`; aligned-reference bases must be a finite nonnegative integer no greater than the configured mitochondrial length and must agree with the reported aligned fraction. SA-tag, primary, supplementary, and secondary indicators must be exact binary values, and `is_primary` must equal the logical absence of secondary and supplementary flags. The required near-complete-alignment QC fraction is trusted only when it is finite and bounded, the upstream QC module and metric statuses are both `ok`, and its declared denominator is exactly `primary_alignment_records`. Any out-of-domain or internally inconsistent required field suppresses categorical interpretation with an explicit `not_evaluable` reason. Raw computable alignment metrics remain reported, and unavailable values are represented as `NA` rather than zero.
...
Internal candidate evidence is treated as a typed analytical contract rather than a permissive tabular input. Every candidate table must contain the generated 14-column observation schema: position, reference and alternate bases, callable depth and its compatibility depth field, alternate count, canonical and compatibility fraction fields, forward and reverse alternate counts, and A/C/G/T counts. Candidate positions must be one-based integers within the configured mitochondrial reference, reference and alternate alleles must be distinct canonical single bases, the reference allele must match the configured FASTA sequence, variant keys must be unique, and the one-selected-alternate-per-site counting model permits at most one candidate row per position. Callable depth must equal both the compatibility depth field and the sum of canonical base counts; alternate counts must equal both the reported alternate-base count and the forward-plus-reverse strand counts. The selected alternate count must equal a largest observed non-reference canonical-base count at that position. The canonical and compatibility alternate-fraction fields must each be finite and bounded, must agree with one another, and must be numerically consistent with `alt_count/callable_depth`. Every analytical module that consumes the candidate table applies this shared contract before interpretation. Present but malformed internal evidence stops the affected analytical step instead of being deduplicated, repaired, dropped, rounded, truncated, or converted to an observed zero.
...
mvTool is disabled by default. Fixture or explicitly requested network mode first validates every existing candidate file against the complete internal candidate-observation contract, including zero-row files; only a complete 14-column header can represent an empty candidate result. Duplicate, partial-header, zero-byte, or otherwise malformed evidence is rejected before a request session is created. External output is accepted as successful only when every submitted candidate has exactly one matching returned input identifier and the response contains no unexpected or duplicate identifiers. Timeout, malformed content, or identity/cardinality mismatch produces `status=unavailable` with a reason code and never fabricates a successful annotation set.

## Exploratory bedMethyl input integrity
Optional bedMethyl rows used by the exploratory methylation layer must identify the configured mitochondrial contig and represent one zero-based base at a time (`end = start + 1`). Coordinates must be nonnegative and fall within the configured mitochondrial reference length. Declared valid coverage, modification counts, canonical counts, supplied other-modification counts, and serialized percentage values must be finite; coverage and count fields must also be nonnegative. Legacy rows that omit the other-modification count may infer only the nonnegative residual of declared valid coverage. Plain-text and gzip-compressed inputs are detected by content. Malformed, nonfinite, wrong-contig, and out-of-bounds rows fail with source and line diagnostics rather than entering an aggregation or becoming an observed zero; missing optional sidecars remain `not_configured`.
