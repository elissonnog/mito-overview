...
Callable depth is the sum of passing canonical `A`, `C`, `G`, and `T` observations. The alternate allele fraction is the largest non-reference canonical count divided by callable depth. A candidate screen is evaluable only when at least one mitochondrial position reaches `MIN_CALLABLE_DEPTH`; an empty candidate table below that coverage threshold is reported as `not_evaluable`, not as evidence that alternate alleles are absent.
