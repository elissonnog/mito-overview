...
Internal candidate evidence is treated as a typed analytical contract rather than a permissive tabular input. Candidate positions must be one-based integers within the configured mitochondrial reference, reference and alternate alleles must be distinct canonical single bases, the reference allele must match the configured FASTA sequence, and candidate keys must be unique where the downstream calculation assumes one row per variant. Callable depth must equal the sum of canonical base counts; alternate counts must equal both the reported alternate-base count and the forward-plus-reverse strand counts; and reported alternate allele fractions must be finite, bounded, and numerically consistent with `alt_count/callable_depth`. Present but malformed internal evidence stops the affected analytical step instead of being dropped, rounded, truncated, or converted to an observed zero.
