...
Circularity edge-depth summaries use the same complete-profile principle. The input must contain the exact integer coordinates `1..MT_LENGTH`, without duplicates or extras, with finite nonnegative depths and finite regional means. Fractional or nonnumeric coordinates are not silently converted to integers. Any malformed present profile produces `not_evaluable/incomplete_depth_profile` and `NA` edge/interior means. Candidate-edge metrics additionally require `status=ok` from the upstream heteroplasmy summary and complete one-based integer candidate coordinates. A failed upstream status suppresses stale candidate metrics; a missing candidate file is distinct from a valid observed zero-candidate table. Primary-read edge metrics require exact binary primary indicators, in-range integer start/end coordinates, ordered start/end pairs for combined soft-clip evaluation, and finite soft-clip fractions in `[0,1]`. Malformed optional evidence produces metric-level `NA/not_evaluable` with an explicit reason while preserving the independent status of a complete depth profile. Deterministic negative tests cover fractional and nonnumeric positions, negative and nonfinite depths, malformed candidate/read coordinates, invalid soft-clip fractions, invalid primary indicators, stale candidates after upstream failure, and missing-versus-observed-zero evidence. Primary tests: `tests/test_circularity_calculations.py`.
...
Candidate `2b981a018629640b81f6a7eb8ec1accab370cbc8` is explicitly rejected and was not pushed. Checkout, exact archive, extracted source distribution, isolated wheel/source-distribution installations, four installed-package smoke workflows, deterministic example rebuilding, and `960/960` tests passed. Reproducibility audit `MOV-REPRO-20260723T074138Z-ef12f565-e795-44d8-9e8e-9f628ce8094e` returned a bounded PASS. Release-engineering audit `MO-RELENG-20260723T074129Z-2ef3043b-27ad-455d-89c9-c6485bd0b419` returned HOLD because an unavailable GitHub immutable-release feature aborted publication instead of activating the approved annotated-tag/hash fallback and because the sdist README contained two relative links to excluded `paper/**` paths. Scientific audit `MO-SCI-2B981A0-20260723T080601Z-CCD0B462` returned HOLD because Phy-Mer could infer a categorical haplogroup from a failed upstream status, duplicate or position-zero evidence, or a REF/FASTA mismatch, while mvTool and circularity could interpret structurally valid but stale candidates and did not distinguish missing evidence from an observed zero-candidate table. One HOLD is sufficient to reject a candidate; no verdict transfers to a changed tree.

The successor working tree now clears heteroplasmy-owned outputs before recounting, propagates upstream heteroplasmy status into all three dependent consumers, applies a complete per-base all-site allele and reference contract before Phy-Mer consensus construction, and separates missing candidate evidence from valid observed zero candidates in mvTool and circularity. GitHub immutable releases remain mandatory when supported; a confirmed endpoint 404 records `fallback=annotated_tag_and_verified_asset_hashes` while preserving every tag, asset, redownload, and SHA-256 gate. README paper references are absolute release-tag URLs so installable source distributions do not contain broken relative links. The first combined remediation suites passed `118/118` scientific tests and `158/158` connected scientific/release tests. Two independent checkout-mode example builds were byte-identical across the 88-file long-read and 74-file short-read bundles; only eight provenance-dependent tracked artifacts required regeneration. The complete pinned suite passed `974/974` in 440.92 seconds, and the installed package passed all four workflow smokes, including standalone BAM and CRAM. These are pre-freeze results only; exact archive/distribution validation and three fresh exact-SHA audits remain pending.
...
## Rejected candidate `223148c`

Candidate `223148c88cac9f483acc9550028ed31c179fb4ed` is rejected release evidence and was not pushed. It completed `974/974` tests, exact archive and source-distribution tests, separate installed wheel/source-distribution smoke workflows, and exact long-/short-read example rebuilding. Those predefined successes did not override three independent HOLD verdicts: release engineering `DA43BF39-6609-434E-AC89-23D37176298A`, reproducibility `MO-REPRO-20260723T091606Z-21a16b95-6d37-4081-86f1-0333ed888412`, and science `MT-AUDIT-223148C-20260723T085745Z`. The reproduced blockers were a fail-open immutable-hosting fallback, reference-filled Phy-Mer classification from almost entirely uncallable mtDNA, acceptance of nonfinite Phy-Mer scores, and stale auxiliary outputs when a run name was reused.

The current successor working tree fails closed for ambiguous GitHub API errors and every enablement failure after endpoint support is known. It requires a configurable Phy-Mer callable-genome fraction, masks eligible residual low-depth positions as `N`, records all eligibility metrics, and rejects nonfinite scores. Normal execution also requires fresh, nonoverlapping run and final destinations; dry-run creates no output. The complete pinned working-tree suite passed `995/995` in 446.64 seconds, an isolated current-tree wheel passed all four workflow modes, and two independent long-/short-read example builds were byte-identical. Only the expected Phy-Mer summary, masked consensus FASTA, and page 13 changed. These changes remain provisional until frozen into a new exact commit and repeated through the exact package, workflow, hygiene, and three-agent audit gates.

## Rejected candidate `7cecb5b`

Candidate `7cecb5ba5e353a6761b981394a942df4c049ce09` is rejected release evidence and was not pushed. Release-engineering audit `MO-RELENG-7CECB5B-20260723T101337Z-95F29A02-DF3B-45AA-86F5-9B75817A9BBF` and reproducibility audit `MO-REPRO-7CECB5B-20260723T100351Z-0854f9c9-d0ec-44c0-8de3-1c87cec8fecd` returned bounded PASS verdicts. Scientific audit `MO-SCI-7CECB5B-20260723T101602Z-164F9485-F02C-4FFA-9E5F-832DCB037A35` returned HOLD because finite Phy-Mer scores outside `[0,1]` were accepted, the lowered synthetic callable-fraction threshold was not fixture-confined, direct-step failure could leave prior categorical outputs, and the official external dependency/import contract was incomplete. One HOLD rejects the candidate, and the two role-specific PASS verdicts do not transfer to changed code.

The successor working tree bounds ranking scores, introduces hash-verified `fixture` versus user-supplied `external` modes, enforces a 95% external callable-genome floor, clears every Phy-Mer-owned output before validation, pins BioPython 1.87, and runs the untouched external script through a narrow Python 3 adapter for its removed `rU` mode. Complete all-site evidence now permits only an exactly reference-matching unresolved `N` in addition to A/C/G/T; that position is reported, excluded from the callable numerator, and masked in the consensus rather than interpreted as a variant. A local untouched-official-tree probe against bundled `NC_012920.1` returned `status=ok`, top haplogroup `H2a2a1`, score `0.999632`, callable positions `16568/16569`, one masked noncanonical-reference position, and consensus `N` at position 3107. This is an execution-compatibility result, not haplogroup-accuracy validation. Focused contracts passed `76/76`. The first complete run then detected only the expected stale page-13 fixture artifact (`1008` passed, one failed); two independent 88-file long-read and 74-file short-read source-mode rebuilds were byte-identical, and only the two expected long-read Phy-Mer artifacts required refresh. The complete rerun passed `1009/1009` in 450.40 seconds. Fresh wheel and source-distribution installations in separate pinned environments outside the checkout each resolved from `site-packages`, exposed all 18 steps, and passed the standard long-read, reduced short-read, no-methylation long-read, and standalone BAM/CRAM workflows. These remain pre-freeze results; all exact-successor gates and three new exact-SHA audits are still mandatory.

## Rejected candidate `9569164`

Candidate `95691642839befa9d3abc935ef0e2c781a783ffd` is rejected release evidence and was not pushed. Release engineering `mito-v030-release-audit-20260723T112050Z-360ac52d-b537-4e7c-8022-c21f79614b45` and reproducibility `93c5d98d-dd20-4e6a-9f28-2bda150a17e6` returned HOLD; science `MITO-V030-9569164-1db1ea4a-35f7-4e52-af9b-d2566878e12a` returned a bounded PASS that cannot approve a modified tree. The exact candidate completed `1009/1009` tests and its package/workflow gates, but GitHub's documented immutable-release `GET 404` response was incorrectly treated as an unsupported-feature fallback, and the public-tag stage could replace validation-packet distribution bytes with independently rebuilt archives.

The successor requires native immutable-release enablement and confirmation before publication and removes the fallback. It binds release publication to the exact packet wheel and source archive, while a clean public-tag build provides only safe member-level equivalence evidence. Exact canonical filenames and regular-file identity are mandatory, and archive traversal, links, special members, unsafe or case-colliding paths, excess inventory, payload drift, and executable-state drift are rejected. The connected remediation suite passed `370/370` in 340.01 seconds, and the complete pre-freeze suite passed `1038/1038` in 452.80 seconds without generated-output or manuscript drift. Exact-successor archive/package/workflow validation and three fresh role-separated PASS audits remain pending; no manuscript values are updated from this provisional working-tree result.

## Rejected candidate `5287009`

Candidate `5287009917db16b4a9bbd14e2fb5caae951e7ef2` is rejected and was not pushed. Its Git archive and source distribution each passed `1038/1038` tests, both installed package forms passed all four synthetic workflow modes, and repeated long-/short-read examples matched the tracked 88-/74-file bundles byte-for-byte. The clean second package build nevertheless failed the mandatory distribution comparison because the verifier incorrectly rejected setuptools' normal nested egg-info `PKG-INFO` in addition to the required canonical root `PKG-INFO`.

The corrected rule requires the canonical root metadata member but permits standard nested egg-info metadata. It retains all path, root, link, special-file, collision, payload, size, and executable-state checks. The focused suite passed `11/11`, comparison of the two real builds passed across 40 wheel and 422 source-archive members, connected affected tests passed `176/176`, and the complete replacement working-tree suite passed `1039/1039` in 432.48 seconds. This correction requires a new commit and exact affected-gate repetition; none of these results approves the rejected SHA or changes the frozen manuscript.

## Rejected candidate `616f798`

Candidate `616f798794931475b5c2abe8352b2520073f934f` is rejected and was not pushed. Its exact source, wheel, source distribution, four workflow modes, and deterministic examples passed before review. Reproducibility audit `MITO-RO-20260723T160529Z-241270f0` returned a bounded PASS. Release-engineering audit `MITO-REL-20260723-616F798-0C7415A4-D0D6-4ED9-A47D-5639BF2E9570` returned HOLD because the release runner prohibited manuscript inputs while still enforcing the frozen manuscript tree. Scientific audit `MITO-SCI-616f798-20260723T160618Z-39adb37f-344b-41a1-8048-6a0de28f0925` returned HOLD because synthetic Phy-Mer fixture output could be promoted to formal identity evidence and nonpositive circularity edge windows were not rejected.
