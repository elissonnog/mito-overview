...
## Rejected candidate `5287009` and canonical sdist metadata remediation

Exact commit `5287009917db16b4a9bbd14e2fb5caae951e7ef2` is rejected and was not pushed. Its Git archive and extracted source distribution each passed `1038/1038` tests; separate wheel and source-distribution installations resolved from `site-packages`, listed all 18 steps, and each passed the four synthetic workflow modes. Two independent long-read and short-read example regenerations were byte-identical to one another and to the tracked 88-file and 74-file bundles. The required second distribution build then exposed an over-strict verifier rule: a standards-conforming setuptools sdist contains the canonical root `PKG-INFO` and an additional `mito_overview.egg-info/PKG-INFO`, but the comparator rejected any second file with that basename. A failed required gate rejects the commit even though package execution passed.

The successor requires the canonical root `mito_overview-0.3.0/PKG-INFO` exactly and still rejects missing root metadata, duplicate or unsafe member paths, and noncanonical archive roots, while permitting standard nested egg-info metadata. The adversarial comparator suite passed `11/11`. Applied to the two actual exact builds, the corrected comparator found exact member equivalence for 40 wheel members and 422 source-archive members; the wheel container was byte-identical, while the sdist container hash differed only in ignored archive metadata and all member paths, payloads, sizes, and executable states matched. Connected affected tests passed `176/176`, and the complete replacement working-tree suite passed `1039/1039` in 432.48 seconds. This is remediation evidence, not approval of `5287009`; a new exact successor and fresh audits are required.

## Rejected candidate `616f798` and manuscript/fixture/circularity remediation

Exact commit `616f798794931475b5c2abe8352b2520073f934f` is rejected and was not pushed. Reproducibility audit `MITO-RO-20260723T160529Z-241270f0` passed its bounded implementation review. Release-engineering audit `MITO-REL-20260723-616F798-0C7415A4-D0D6-4ED9-A47D-5639BF2E9570` held the candidate because the GitHub-only runner contradicted its own interface by making the frozen `paper/` tree an acceptance gate. Scientific audit `MITO-SCI-616f798-20260723T160618Z-39adb37f-344b-41a1-8048-6a0de28f0925` held it because synthetic Phy-Mer fixture output could be labeled as a formal assignment and `edge_window <= 0` could produce misleading circularity status. One HOLD rejects the candidate; no PASS transfers to a changed tree.

The successor removes the manuscript-tree release check while leaving `paper/**` unchanged. It converts synthetic fixture scope into `formal_haplogroup_assignment_status=not_evaluable`, reason `synthetic_phymer_fixture_not_biological`, and best match `NA` in identity QC, while preserving external classifier semantics. Circularity requires a positive integer edge window before any output is created. The focused suite passed `74/74`. The first full pinned suite passed `1041` tests and correctly detected only two stale tracked long-read artifacts, `mito_identity_qc_summary.tsv` and `09_mito_identity_qc.html`; regeneration changed only those files and did not alter the 74-file short-read bundle. The clean complete rerun passed `1042/1042` in 472.39 seconds. A new exact-commit gate cycle remains pending.

## Rejected candidate `db62aa4` and contract-boundary remediation
