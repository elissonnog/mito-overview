...
Each clean-room platform matrix starts from the sealed raw FASTQs, rebuilds the pooled GM11906 and seeded GM12878 derivatives and alignments, and then uses each newly generated BAM for the two within-matrix default report invocations. Exact normalized-table repeatability is therefore evaluated separately from cross-platform reconstruction of the deterministic derivatives. Each public asset directory documents its filter profiles, exact status values, and selected report artifacts. Version-bound evidence is available from the [`v0.3.1` GitHub release](https://github.com/elissonnog/mito-overview/releases/tag/v0.3.1).
